...
private data class Position(val x: Int, val y: Int) {
    ...
    fun getValidLongCheatPositions(map: Map): List<Position> =
...
private fun getLongCheats(path: Path, map: Map, memo: HashMap<Position, Int>): List<Cheat> = path
    .flatMapIndexed { i, position ->
        position.getValidLongCheatPositions(map)
            .map { cheatEndPosition ->
                val cheatTime = when {
                    memo.containsKey(cheatEndPosition) -> memo[cheatEndPosition]!!
                    else -> getTiming(cheatEndPosition, path.last(), map)
...
fun main() {
    ...
    fun part2(input: String): Int {
        ...
        return getLongCheats(initialPath, map, memo)
            .count { it.timeSaved >= 100 }
    }

    val input = readAllInput("Day20")
    part1(input).println()
    part2(input).println()
}
