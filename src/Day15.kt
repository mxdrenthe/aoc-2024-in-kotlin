...
private data class State(val map: Map) {
    private fun getRobotPosition(): Position =
        map.filterValues { it == EntityType.ROBOT }.keys.first()
    ...
    private fun getEntitiesToMoveVertical(direction: Move): List<Position> {
        ...
        while (true) {
            ...
            val allEmpty =
                currHorizon.all { map[it.getNextPosition(direction)] == EntityType.EMPTY }
            if (allEmpty) {
                entities.addAll(
                    entities
...
fun main() {
    ...
    val testInput = readAllInput("Day15_test")
    check(part2(testInput) == 9021L)

    val input = readAllInput("Day15")
    part2(input).println()
}
